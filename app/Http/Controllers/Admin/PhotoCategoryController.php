...
{
    protected $redirect_index = '/admin/photo-categories/index';

    protected $pre_uri = '/admin/photo-categories/';

    protected $view_path = 'admin.photo-categories.';

    protected $model_name = '图片分类';

    protected $model;

    public function __construct()
    {
        $this->model = new PhotoCategory();
    }
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->input('data'), [
            'name' => 'unique:photo_categories,name'
        ], [
            'name.unique' => '分类名已存在！'
        ]);
        if($validator->fails()) {
